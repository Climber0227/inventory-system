...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await request.put(`/supplier/${form.value.id}`, form.value)
  ...
  <view class="page">
    <view class="section">
      <view class="form-item">
        ...
        <input v-model="form.name" class="input" placeholder="请输入供应商名称" />
      </view>
      <view class="form-item">
        <text class="label">供应商编码</text>
        <input v-model="form.code" class="input" placeholder="请输入供应商编码" />
      </view>
      <view class="form-item">
