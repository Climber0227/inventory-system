...
function goEdit(id) { uni.navigateTo({ url: `/pages/customer/edit?id=${id}` }) }
function goCreate() { uni.navigateTo({ url: '/pages/customer/edit' }) }
...
<template>
  <view class="page">
    ...
    <view class="search-bar">
      <input v-model="keyword" class="search-input" placeholder="搜索客户名称" @confirm="onSearch" @input="onInput" />
    </view>
    ...
    <view v-else>
      ...
      <view v-if="list.length === 0 && !loading" class="empty">暂无客户</view>
    </view>
    <FloatingHome />
  </view>
</template>
