...
export const navTree: NavItem[] = [
  {
    id: 'role-guide', title: '角色使用指南', icon: 'UserFilled',
    children: [
      { id: 'admin-guide', title: '管理员使用指南' },
      { id: 'employee-guide', title: '员工使用指南' },
    ],
  },
    ...
    content: [
      {
        type: 'text',
        title: '管理员角色说明',
        body: '管理员（role_1）拥有系统的全部操作权限。您可以看到所有菜单、审核所有单据、管理系统配置和用户账号。非管理员看不到的模块：采购管理、审批管理、系统管理（用户/日志/配置/回收站）。',
      },
      {
        type: 'text',
        title: '第一步：初始化系统',
        body: '① 登录系统后，先去「系统管理 → 员工管理」修改默认密码，并为团队其他成员创建账号（仓库员选"员工"角色）。\n② 进入「仓库管理」，按实际场地建立仓库层级结构。建完1级仓库后，点击该节点逐层往下建子仓库。注意只有4级叶子节点可以存放库存。\n③ 进入「商品管理」，新建商品资料，或通过「导入Excel」批量导入。\n④ 进入「供应商管理」和「客户管理」，录入常用的供应商和客户信息。',
      },
      {
        type: 'text',
        title: '第二步：首次入库',
        body: '① 进入「采购管理 → 新建入库单」，选择供应商、目标仓库（叶子节点）、入库商品及数量/单价。\n② 提交审批（状态变为"待审核"）。\n③ 回到采购列表，找到该单据，点击「审核通过」。\n④ 审核通过后库存立即生效，可在「库存查询」中查看。',
      },
      {
        type: 'text',
        title: '第三步：日常审核',
        body: '员工提交的采购单、销售单、盘点单、调拨单，都需要管理员审核。\n• 进入「待审批」页面，集中查看所有待审核单据，每个标签页有数量角标。\n• 审核时请核对数量、仓库、商品是否正确。\n• 审核通过后库存立即生效，驳回后单据退回草稿状态（员工可重新编辑提交）。',
      },
      {
        type: 'flow',
        title: '审批流程一览',
        steps: [
          { from: '员工创建草稿', arrow: '→', to: '提交审批', desc: '单据变为"待审核"' },
          { from: '待审核', arrow: '→', to: '审核通过', desc: '管理员操作，库存生效' },
          { from: '待审核', arrow: '→', to: '驳回', desc: '管理员操作，退回到草稿' },
        ],
      },
      {
        type: 'text',
        title: '第四步：数据维护',
        body: '• 商品价格调整：进入「商品管理」编辑商品的采购价/销售价，历史单据不受影响。\n• 库存盘点：员工提交盘点单 → 管理员审核通过 → 再点击"执行调整"更新库存。注意：审核通过≠库存已调整，必须手动执行调整。\n• 库存调拨：审核通过后源仓库扣减、目标仓库增加，调拨后的商品沿用源批次的成本价。\n• 数据导出：大部分列表页支持导出Excel，商品和库存的导出仅管理员可用。',
      },
      {
        type: 'text',
        title: '第五步：日常监控',
        body: '• 工作台：登录后首页可看到今日入库/出库统计、库存预警、最近操作动态。\n• 统计报表：查看入库汇总、出库汇总、库存周转率、库存预警。\n• 操作日志：在「系统管理 → 操作日志」中可追溯所有用户的操作记录。\n• 已作废列表：在「系统管理 → 已作废列表」中统一查看所有作废的单据。',
      },
      {
        type: 'warning',
        title: '管理员注意事项',
        body: '• 审核通过后库存立即变化，无法撤回，请仔细核对后再操作。\n• 已入库/已出库的单据不可取消，只能作废（作废不会回滚库存）。\n• 作废操作不可逆，确认前请三思。\n• 不要随意修改系统配置，可能导致系统异常。\n• admin 账号不可删除或禁用。',
      },
    ],
  },
  'employee-guide': {
    ...
    content: [
      {
        type: 'text',
        title: '员工角色说明',
        body: '员工（role_2）拥有日常业务操作权限。您可以：新建单据（采购/销售/盘点/调拨）、编辑草稿、提交审批、查看库存、导出数据。您看不到的：采购价/成本价、采购管理模块、系统管理菜单、审批管理页面。',
      },
      {
        type: 'text',
        title: '第一步：登录与熟悉界面',
        body: '① 使用管理员分配的账号密码登录系统。\n② 首次登录建议修改密码（点击右上角头像 → 个人设置）。\n③ 左侧菜单是您可操作的所有模块，灰色或不可见的菜单说明无权限。\n④ 工作台显示今日业务概览和库存预警，关注预警信息及时补货。',
      },
      {
        type: 'text',
        title: '第二步：日常出库操作',
        body: '① 进入「销售管理 → 新建出库单」，选择客户、出库仓库。\n② 选完仓库后，商品下拉列表会显示该仓库所有有库存的商品（按批次分开显示）。\n③ 选择商品和数量，系统会自动带入销售价。\n④ 保存草稿后可以在列表页继续编辑，确认无误后提交审批。\n⑤ 提交后等待管理员审核，审核通过后库存自动扣减。',
      },
      {
        type: 'text',
        title: '第三步：库存盘点操作',
        body: '① 进入「库存盘点 → 新建盘点单」，选择要盘点的仓库。\n② 系统自动加载该仓库所有有库存的商品（按批次显示，每个批次一行）。\n③ 逐项录入实盘数量，差异数自动计算（正数盘盈、负数盘亏）。\n④ 提交后管理员审核通过，再执行调整更新库存。',
      },
      {
        type: 'text',
        title: '第四步：库存调拨操作',
        body: '① 进入「库存调拨 → 新建调拨单」，选择调出仓库和调入仓库（两者不能相同）。\n② 选择商品时，下拉列表仅显示调出仓库有库存的商品。\n③ 可指定具体批次调拨，不指定则按后进先出自动扣减。\n④ 提交审批，管理员审核通过后库存从调出仓转移到调入仓。',
      },
      {
        type: 'text',
        title: '第五步：库存查询',
        body: '• 树形浏览：默认按仓库层级展示，点击▶展开子仓库，叶子节点显示库存明细。\n• 商品搜索：输入商品名称或编码，跨仓库搜索该商品的分布情况。\n• 仓库筛选：点击"全部仓库"可筛选特定仓库的库存。\n• 库存流水：点击右上角「流水」查看所有库存变动记录。\n• 注意：员工角色看不到成本价和成本金额。',
      },
      {
        type: 'note',
        title: '与小程序配合使用',
        body: 'PC端和小程序端数据互通。仓库员在现场用小程序的扫码功能快速选择商品、移动端做盘点和出库，管理员在PC端集中审核。切换设备不会丢失数据，草稿可以在两端编辑。',
      },
      {
        type: 'warning',
        title: '员工注意事项',
        body: '• 出库时注意核对仓库和商品，选错仓库会导致出库失败（库存不足）。\n• 盘点时实盘数量要准确填写，差异会直接影响库存。\n• 单据提交审批后不能编辑，如需修改请联系管理员驳回。\n• 取消操作（取消出库/取消入库）不可逆，操作前仔细确认。\n• 如遇到按钮灰色不可点击，说明当前单据状态不允许该操作，或您无此权限。',
      },
    ],
  },
  dashboard: {
  ...
  'product-list': {
    ...
    content: [
      ...
      },
      {
        type: 'text',
        title: '导入 Excel',
        body: '• 下载模板：点击「下载模板」获取空白模板文件，按格式填写商品数据。\n• 导入：上传填好的 Excel，系统会自动识别：商品名称已存在 → 更新（价格、库存上下限等）；不存在 → 新建。',
      },
      {
        type: 'warning',
        title: '⚠️ 两种导出，功能完全不同！请仔细区分',
        body: '导出按钮旁边有个下拉小箭头（▼），点击后可切换两种导出模式：\n\n🔴 「导出规范Excel」— 可以回导的格式（含商品编码），修改后可直接重新导入系统。用于批量修改商品信息的场景。\n\n🔵 「导出Excel」— 普通导出，包含库存数量、预警状态等完整信息，仅用于查看或打印，不能回导！\n\n❌ 常见错误：点了"导出Excel"，修改后又尝试导入，结果失败。\n✅ 正确做法：如果要批量修改商品后重新导入，必须选"导出规范Excel"。',
      },
      {
        type: 'text',
        title: '导出筛选结果',
        body: '在筛选栏最右侧的绿色按钮，导出当前筛选条件下的商品数据（不受表格分页影响，导出的是全部匹配数据）。',
      },
    ],
  },
  'product-category': {
